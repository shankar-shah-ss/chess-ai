# opening_book.py - Comprehensive Opening Book System with Lichess Integration
import json
import sqlite3
import hashlib
import random
...
class BookMove:
    ...
    source: str = "local"  # Source: local, lichess, combined
...
class PGNDatabase:
    ...
    def _init_database(self):
        ...
        try:
            ...
            ''')
            self.conn.commit()
            logger.debug(f"PGN database initialized: {self.db_path}")
        except Exception as e:
...
class LichessRealTimeClient:
    """Real-time client for Lichess masters database"""
    
    def __init__(self):
        self.base_url = "https://explorer.lichess.ovh"
        self.session = requests.Session()
        self.session.headers.update({
            'User-Agent': 'ChessAI-HybridOpening/1.0'
        })
    ...
    def get_position_data(self, fen: str) -> Optional[Dict]:
        """Get position data from Lichess with caching"""
        if not self.enabled:
            return None
    ...
    def convert_to_book_moves(self, lichess_data: Dict) -> List[BookMove]:
        """Convert Lichess data to BookMove objects"""
        ...
        for move_data in lichess_data['moves']:
            try:
                ...
                if total_games < 10:  # Skip moves with very few games
                    continue
    ...
    def disable(self):
        """Disable Lichess client (for offline mode)"""
        self.enabled = False
    
    def enable(self):
        """Enable Lichess client"""
        self.enabled = True

class OpeningBook:
    """Hybrid opening book system combining multiple sources with Lichess integration"""
    ...
    def __init__(self, config: Dict[str, Any] = None):
        ...
        self.enable_lichess = self.config.get('enable_lichess', True)
        self.lichess_fallback = self.config.get('lichess_fallback', True)
        self.prefer_local_threshold = self.config.get('prefer_local_threshold', 100)
        self.lichess_min_games = self.config.get('lichess_min_games', 10)
        ...
        if self.enable_lichess:
            try:
                self.lichess_client = LichessRealTimeClient()
                logger.debug("Lichess real-time client initialized")
            except Exception as e:
                logger.debug(f"Failed to initialize Lichess client: {e}")
    ...
    def _initialize_books(self):
        ...
        try:
            ...
            logger.debug("Opening book system initialized successfully")
        except Exception as e:
    ...
    def _create_essential_openings(self):
        ...
        # Starting position - First moves (REBALANCED for more variety)
        essential_openings[STARTING_FEN] = [
            BookMove("e2e4", 800, 320, 240, 240, 2220, 0),   # King's Pawn (reduced weight)
            BookMove("d2d4", 800, 320, 240, 240, 2200, 0),   # Queen's Pawn (increased weight)
            BookMove("g1f3", 700, 280, 210, 210, 2180, 0),   # Reti Opening (increased weight)
            BookMove("c2c4", 600, 240, 180, 180, 2170, 0),   # English Opening (increased weight)
            BookMove("b1c3", 400, 160, 120, 120, 2120, 0),   # Van't Kruijs Opening (increased weight)
            BookMove("f2f4", 350, 140, 105, 105, 2100, 0),   # Bird's Opening (increased weight)
            BookMove("g2g3", 300, 120, 90, 90, 2110, 0),     # Benko's Opening (increased weight)
            BookMove("b2b3", 250, 100, 75, 75, 2090, 0),     # Nimzowitsch-Larsen Attack (increased weight)
            BookMove("e2e3", 120, 50, 40, 30, 2080, 0),      # Van't Kruijs Opening
            BookMove("d2d3", 100, 42, 32, 26, 2070, 0),      # Mieses Opening
            BookMove("h2h3", 80, 35, 25, 20, 2060, 0),       # Clemenz Opening
            BookMove("a2a3", 70, 30, 22, 18, 2050, 0),       # Anderssen Opening
            BookMove("c2c3", 60, 25, 20, 15, 2040, 0),       # Saragossa Opening
            BookMove("h2h4", 50, 20, 15, 15, 2030, 0),       # Desprez Opening
            BookMove("a2a4", 45, 18, 14, 13, 2020, 0),       # Ware Opening
            BookMove("g1h3", 40, 16, 12, 12, 2010, 0),       # Amar Opening
            BookMove("f2f3", 35, 14, 11, 10, 2000, 0),       # Barnes Opening
            BookMove("b1a3", 30, 12, 9, 9, 1990, 0),         # Sodium Attack
            BookMove("g2g4", 25, 10, 8, 7, 1980, 0),         # Grob's Attack
            BookMove("b2b4", 20, 8, 6, 6, 1970, 0),          # Polish Opening
        ]
        
        # === 1.e4 OPENINGS ===
        
        # After 1.e4 - Black responses (MASSIVE EXPANSION - 20 moves)
        ...
        essential_openings[fen_1e4] = [
            ...
            BookMove("b7b6", 120, 48, 36, 36, 2110, 0),      # Owen's Defense
            BookMove("a7a6", 100, 40, 30, 30, 2100, 0),      # St. George Defense
            BookMove("h7h6", 80, 32, 24, 24, 2090, 0),       # Carr Defense
            BookMove("f7f5", 70, 28, 21, 21, 2080, 0),       # Barnes Defense
            BookMove("a7a5", 60, 24, 18, 18, 2070, 0),       # Ware Defense
            BookMove("b8a6", 50, 20, 15, 15, 2060, 0),       # Lemming Defense
            BookMove("h7h5", 45, 18, 14, 13, 2050, 0),       # Goldsmith Defense
            BookMove("f7f6", 40, 16, 12, 12, 2040, 0),       # Duras Gambit
            BookMove("b7b5", 35, 14, 11, 10, 2030, 0),       # Polish Defense
            BookMove("g8h6", 30, 12, 9, 9, 2020, 0),         # Paris Defense
            BookMove("c8e6", 25, 10, 8, 7, 2010, 0),         # Rat Defense
        ]
        
        # === KING'S PAWN GAME (1.e4 e5) ===
        
        # After 1.e4 e5 - White's second move (MASSIVE EXPANSION - 18 moves)
        ...
        essential_openings[fen_1e4_e5] = [
            ...
            BookMove("d2d4", 120, 48, 36, 36, 2130, 0),      # Center Game
            BookMove("f1e2", 100, 40, 30, 30, 2120, 0),      # Hungarian Opening
            BookMove("c2c3", 80, 32, 24, 24, 2110, 0),       # Lopez Opening
            BookMove("g2g3", 70, 28, 21, 21, 2100, 0),       # Benko's Opening
            BookMove("b2b3", 60, 24, 18, 18, 2090, 0),       # Nimzowitsch-Larsen
            BookMove("a2a3", 50, 20, 15, 15, 2080, 0),       # Mengarini Opening
            BookMove("h2h3", 45, 18, 14, 13, 2070, 0),       # Tayler Opening
            BookMove("c2c4", 40, 16, 12, 12, 2060, 0),       # English Opening
            BookMove("d1e2", 35, 14, 11, 10, 2050, 0),       # Napoleon Attack
            BookMove("f1d3", 30, 12, 9, 9, 2040, 0),         # Philidor Opening
            BookMove("b1a3", 25, 10, 8, 7, 2030, 0),         # Paris Opening
            BookMove("a2a4", 20, 8, 6, 6, 2020, 0),          # Ware Opening
            BookMove("h2h4", 15, 6, 5, 4, 2010, 0),          # Wayward Queen Attack
        ]
        
        # === ENGLISH OPENING COVERAGE (CRITICAL MISSING POSITIONS!) ===
        
        # After 1.c4 - Black's response
        fen_1c4 = "rnbqkbnr/pppppppp/8/8/2P5/8/PP1PPPPP/RNBQKBNR b KQkq - 0 1"
        essential_openings[fen_1c4] = [
            BookMove("e7e5", 600, 240, 180, 180, 2200, 0),   # Reversed Sicilian
            BookMove("g8f6", 500, 200, 150, 150, 2180, 0),   # Anglo-Indian
            BookMove("e7e6", 400, 160, 120, 120, 2160, 0),   # Anglo-French
            BookMove("c7c5", 350, 140, 105, 105, 2150, 0),   # Symmetrical English
            BookMove("d7d5", 300, 120, 90, 90, 2140, 0),     # Anglo-Scandinavian
            BookMove("c7c6", 250, 100, 75, 75, 2130, 0),     # Anglo-Slav
            BookMove("f7f5", 200, 80, 60, 60, 2120, 0),      # Dutch setup
            BookMove("b7b6", 150, 60, 45, 45, 2110, 0),      # English Defense
        ]
        
        # After 1.c4 e5 - White's response (Reversed Sicilian)
        fen_1c4_e5 = "rnbqkbnr/pppp1ppp/8/4p3/2P5/8/PP1PPPPP/RNBQKBNR w KQkq - 0 2"
        essential_openings[fen_1c4_e5] = [
            BookMove("b1c3", 500, 200, 150, 150, 2200, 0),   # Closed Sicilian setup
            BookMove("g1f3", 450, 180, 135, 135, 2190, 0),   # Open Sicilian setup
            BookMove("g2g3", 300, 120, 90, 90, 2170, 0),     # Fianchetto setup
            BookMove("e2e3", 250, 100, 75, 75, 2160, 0),     # Quiet development
            BookMove("e2e4", 200, 80, 60, 60, 2150, 0),      # Transpose to King's Pawn
            BookMove("d2d3", 150, 60, 45, 45, 2140, 0),      # King's Indian Attack
        ]
        
        # After 1.c4 e5 2.Nf3 - Black's response (THE MISSING POSITION!)
        fen_english_nf3 = "rnbqkbnr/pppp1ppp/8/4p3/2P5/5N2/PP1PPPPP/RNBQKB1R b KQkq - 1 2"
        essential_openings[fen_english_nf3] = [
            BookMove("b8c6", 600, 240, 180, 180, 2200, 0),   # Natural development
            BookMove("d7d6", 400, 160, 120, 120, 2180, 0),   # Closed setup
            BookMove("g8f6", 350, 140, 105, 105, 2170, 0),   # King's Indian setup
            BookMove("f7f5", 300, 120, 90, 90, 2160, 0),     # Dutch setup
            BookMove("f8c5", 250, 100, 75, 75, 2150, 0),     # Bishop development
            BookMove("b8d7", 200, 80, 60, 60, 2140, 0),      # Flexible development
            BookMove("c7c6", 150, 60, 45, 45, 2130, 0),      # Caro-Kann setup
            BookMove("a7a6", 120, 48, 36, 36, 2120, 0),      # Waiting move
        ]
        
        # After 1.c4 Nf6 - White's response (Anglo-Indian)
        fen_1c4_nf6 = "rnbqkb1r/pppppppp/5n2/8/2P5/8/PP1PPPPP/RNBQKBNR w KQkq - 1 2"
        essential_openings[fen_1c4_nf6] = [
            BookMove("b1c3", 500, 200, 150, 150, 2200, 0),   # Four Knights setup
            BookMove("g1f3", 450, 180, 135, 135, 2190, 0),   # Reti setup
            BookMove("g2g3", 350, 140, 105, 105, 2180, 0),   # King's Indian Attack
            BookMove("d2d4", 300, 120, 90, 90, 2170, 0),     # Transpose to d4
            BookMove("e2e4", 200, 80, 60, 60, 2160, 0),      # Transpose to e4
        ]
        
        # CRITICAL: After 1.e4 e5 2.Nf3 - Black's response (MISSING POSITION!)
        fen_after_nf3 = "rnbqkbnr/pppp1ppp/8/4p3/4P3/5N2/PPPP1PPP/RNBQKB1R b KQkq - 1 2"
        essential_openings[fen_after_nf3] = [
            BookMove("b8c6", 800, 320, 240, 240, 2200, 0),   # Knight to c6 (most common)
            BookMove("g8f6", 600, 240, 180, 180, 2180, 0),   # Petrov Defense
            BookMove("d7d6", 400, 160, 120, 120, 2160, 0),   # Philidor Defense
            BookMove("f7f5", 300, 120, 90, 90, 2140, 0),     # Latvian Gambit
            BookMove("f8c5", 250, 100, 75, 75, 2130, 0),     # Italian Game setup
            BookMove("b8d7", 200, 80, 60, 60, 2120, 0),      # Paris Defense
            BookMove("d8f6", 150, 60, 45, 45, 2110, 0),      # Damiano Defense
            BookMove("f8d6", 120, 48, 36, 36, 2100, 0),      # Elephant Gambit
            BookMove("g7g6", 100, 40, 30, 30, 2090, 0),      # Modern Defense
            BookMove("f8e7", 80, 32, 24, 24, 2080, 0),       # Hungarian Defense
        ]
        
        # RUY LOPEZ (1.e4 e5 2.Nf3 Nc6 3.Bb5) - MASSIVE EXPANSION (18 moves)
        ...
        essential_openings[fen_ruy_lopez] = [
            ...
            BookMove("f8c5", 80, 32, 24, 24, 2130, 0),       # Classical Defense
            BookMove("d7d6", 70, 28, 21, 21, 2120, 0),       # Steinitz Defense Modern
            BookMove("b7b5", 60, 24, 18, 18, 2110, 0),       # Jaenisch Gambit
            BookMove("f8e7", 50, 20, 15, 15, 2100, 0),       # Hungarian Defense
            BookMove("c8d7", 45, 18, 14, 13, 2090, 0),       # Cozio Defense
            BookMove("d8f6", 40, 16, 12, 12, 2080, 0),       # Bird's Defense
            BookMove("c6e7", 35, 14, 11, 10, 2070, 0),       # Cozio Defense Modern
            BookMove("c6d4", 30, 12, 9, 9, 2060, 0),         # Brentano Defense
            BookMove("h7h6", 25, 10, 8, 7, 2050, 0),         # Smyslov Defense
            BookMove("c6a5", 22, 9, 7, 6, 2040, 0),          # Pollock Defense
            BookMove("d8e7", 20, 8, 6, 6, 2030, 0),          # Rotlewi Defense
            BookMove("c8g4", 18, 7, 6, 5, 2020, 0),          # Alapin Defense
            BookMove("h7h5", 15, 6, 5, 4, 2010, 0),          # Graz Defense
        ]
        
        # ITALIAN GAME (1.e4 e5 2.Nf3 Nc6 3.Bc4) - MASSIVE EXPANSION (18 moves)
        ...
        essential_openings[fen_italian] = [
            ...
            BookMove("f8e7", 120, 48, 36, 36, 2140, 0),      # Hungarian Defense
            BookMove("d7d6", 100, 40, 30, 30, 2130, 0),      # Paris Defense
            BookMove("g7g6", 80, 32, 24, 24, 2120, 0),       # Fianchetto Defense
            BookMove("a7a6", 70, 28, 21, 21, 2110, 0),       # Blackburne Shilling Gambit
            BookMove("b7b5", 60, 24, 18, 18, 2100, 0),       # Evans Gambit Declined
            BookMove("c6d4", 50, 20, 15, 15, 2090, 0),       # Blackburne Shilling Gambit
            BookMove("h7h6", 45, 18, 14, 13, 2080, 0),       # Anti-Fried Liver
            BookMove("c8e6", 40, 16, 12, 12, 2070, 0),       # Center Attack
            BookMove("d8f6", 35, 14, 11, 10, 2060, 0),       # Rousseau Gambit
            BookMove("c6a5", 30, 12, 9, 9, 2050, 0),         # Blackburne Shilling Gambit
            BookMove("d8e7", 25, 10, 8, 7, 2040, 0),         # Legall's Mate Defense
            BookMove("c8d7", 22, 9, 7, 6, 2030, 0),          # Lopez Defense
            BookMove("b8d7", 20, 8, 6, 6, 2020, 0),          # Paris Defense
            BookMove("h7h5", 18, 7, 6, 5, 2010, 0),          # Damiano Defense
        ]
        
        # === SICILIAN DEFENSE (1.e4 c5) ===
        
        # After 1.e4 c5 - White's second move (MASSIVE EXPANSION - 20 moves)
        ...
        essential_openings[fen_sicilian] = [
            ...
            BookMove("d2d3", 80, 32, 24, 24, 2130, 0),       # King's Indian Attack
            BookMove("g2g3", 70, 28, 21, 21, 2120, 0),       # Fianchetto Variation
            BookMove("f1c4", 60, 24, 18, 18, 2110, 0),       # Italian System
            BookMove("b2b4", 50, 20, 15, 15, 2100, 0),       # Wing Gambit
            BookMove("h2h3", 45, 18, 14, 13, 2090, 0),       # Mengarini Attack
            BookMove("a2a3", 40, 16, 12, 12, 2080, 0),       # Mengarini Attack
            BookMove("c2c4", 35, 14, 11, 10, 2070, 0),       # English Attack
            BookMove("d2d4", 30, 12, 9, 9, 2060, 0),         # Center Game
            BookMove("b1a3", 25, 10, 8, 7, 2050, 0),         # Lemming Attack
            BookMove("f1e2", 22, 9, 7, 6, 2040, 0),          # Closed System
            BookMove("d1e2", 20, 8, 6, 6, 2030, 0),          # Napoleon Attack
            BookMove("a2a4", 18, 7, 6, 5, 2020, 0),          # Maroczy Bind
            BookMove("h2h4", 15, 6, 5, 4, 2010, 0),          # Keres Attack
            BookMove("b2b3", 12, 5, 4, 3, 2000, 0),          # Snyder Variation
            BookMove("g1e2", 10, 4, 3, 3, 1990, 0),          # Chameleon System
        ]
        
        # SICILIAN NAJDORF (1.e4 c5 2.Nf3 d6 3.d4 cxd4 4.Nxd4 Nf6 5.Nc3 a6) - MASSIVE EXPANSION (18 moves)
        ...
        essential_openings[fen_najdorf] = [
            ...
            BookMove("f1c4", 120, 48, 36, 36, 2170, 0),      # Fischer Attack
            BookMove("a2a4", 100, 40, 30, 30, 2160, 0),      # Pawn storm
            BookMove("c1d2", 80, 32, 24, 24, 2150, 0),       # Positional setup
            BookMove("d1d2", 70, 28, 21, 21, 2140, 0),       # Queen development
            BookMove("b2b3", 60, 24, 18, 18, 2130, 0),       # Fianchetto prep
            BookMove("c1g5", 50, 20, 15, 15, 2120, 0),       # Pin variation
            BookMove("h2h4", 45, 18, 14, 13, 2110, 0),       # Aggressive attack
            BookMove("d1f3", 40, 16, 12, 12, 2100, 0),       # Queen attack
            BookMove("a2a3", 35, 14, 11, 10, 2090, 0),       # Quiet development
            BookMove("f1d3", 30, 12, 9, 9, 2080, 0),         # Bishop development
            BookMove("d1e2", 25, 10, 8, 7, 2070, 0),         # Queen to e2
            BookMove("c1f4", 22, 9, 7, 6, 2060, 0),          # Bishop to f4
            BookMove("g1e2", 20, 8, 6, 6, 2050, 0),          # Knight to e2
        ]
        
        # === FRENCH DEFENSE (1.e4 e6) ===
        
        # After 1.e4 e6 - White's second move (MASSIVE EXPANSION - 18 moves)
        ...
        essential_openings[fen_french] = [
            ...
            BookMove("d2d3", 120, 48, 36, 36, 2150, 0),      # King's Indian Attack
            BookMove("f1e2", 100, 40, 30, 30, 2140, 0),      # Réti Gambit
            BookMove("c2c4", 80, 32, 24, 24, 2130, 0),       # English Attack
            BookMove("g2g3", 70, 28, 21, 21, 2120, 0),       # Fianchetto
            BookMove("f1d3", 60, 24, 18, 18, 2110, 0),       # Diemer-Duhm Gambit
            BookMove("b2b3", 50, 20, 15, 15, 2100, 0),       # Wing Gambit
            BookMove("c2c3", 45, 18, 14, 13, 2090, 0),       # Two Knights
            BookMove("h2h3", 40, 16, 12, 12, 2080, 0),       # Quiet development
            BookMove("a2a3", 35, 14, 11, 10, 2070, 0),       # Quiet development
            BookMove("d1e2", 30, 12, 9, 9, 2060, 0),         # Chigorin Variation
            BookMove("f1c4", 25, 10, 8, 7, 2050, 0),         # Wing Gambit
            BookMove("b1a3", 22, 9, 7, 6, 2040, 0),          # Reti System
            BookMove("h2h4", 20, 8, 6, 6, 2030, 0),          # Steiner Variation
            BookMove("a2a4", 18, 7, 6, 5, 2020, 0),          # Wing Attack
        ]
        ...
        # After 1.d4 - Black responses (MASSIVE EXPANSION - 20 moves)
        ...
        essential_openings[fen_1d4] = [
            ...
            BookMove("c7c6", 120, 48, 36, 36, 2130, 0),      # Slav Defense setup
            BookMove("b7b6", 100, 40, 30, 30, 2120, 0),      # Queen's Indian setup
            BookMove("e7e5", 80, 32, 24, 24, 2110, 0),       # Englund Gambit
            BookMove("b8a6", 70, 28, 21, 21, 2100, 0),       # Mikenas Defense
            BookMove("h7h6", 60, 24, 18, 18, 2090, 0),       # Kadas Opening
            BookMove("a7a6", 50, 20, 15, 15, 2080, 0),       # Gedult Opening
            BookMove("d7d6", 45, 18, 14, 13, 2070, 0),       # Wade Defense
            BookMove("b7b5", 40, 16, 12, 12, 2060, 0),       # Polish Defense
            BookMove("h7h5", 35, 14, 11, 10, 2050, 0),       # Kadas Opening
            BookMove("c8f5", 30, 12, 9, 9, 2040, 0),         # London System Defense
            BookMove("a7a5", 25, 10, 8, 7, 2030, 0),         # Ware Defense
            BookMove("g8h6", 22, 9, 7, 6, 2020, 0),          # Lemming Defense
            BookMove("f7f6", 20, 8, 6, 6, 2010, 0),          # Barnes Defense
        ]
        
        # === QUEEN'S GAMBIT (1.d4 d5 2.c4) ===
        
        # After 1.d4 d5 2.c4 - Black responses (MASSIVE EXPANSION - 18 moves)
        ...
        essential_openings[fen_queens_gambit] = [
            ...
            BookMove("g8f6", 120, 48, 36, 36, 2150, 0),      # Marshall Defense
            BookMove("c8f5", 100, 40, 30, 30, 2140, 0),      # Baltic Defense
            BookMove("b7b6", 80, 32, 24, 24, 2130, 0),       # Queen's Indian setup
            BookMove("g7g6", 70, 28, 21, 21, 2120, 0),       # Grünfeld setup
            BookMove("a7a6", 60, 24, 18, 18, 2110, 0),       # Symmetrical Defense
            BookMove("f7f6", 50, 20, 15, 15, 2100, 0),       # Blackmar-Diemer declined
            BookMove("h7h6", 45, 18, 14, 13, 2090, 0),       # Waiting move
            BookMove("b8a6", 40, 16, 12, 12, 2080, 0),       # Mikenas Defense
            BookMove("c8e6", 35, 14, 11, 10, 2070, 0),       # Baltic Defense
            BookMove("b8d7", 30, 12, 9, 9, 2060, 0),         # Marshall Defense
            BookMove("a7a5", 25, 10, 8, 7, 2050, 0),         # Ware Defense
            BookMove("h7h5", 22, 9, 7, 6, 2040, 0),          # Kadas Defense
            BookMove("f7f5", 20, 8, 6, 6, 2030, 0),          # Staunton Gambit
        ]
        
        # QUEEN'S GAMBIT DECLINED (1.d4 d5 2.c4 e6) - EXPANDED
        ...
        essential_openings[fen_qgd] = [
            ...
            BookMove("c1f4", 120, 48, 36, 36, 2160, 0),      # London System
            BookMove("c1g5", 100, 40, 30, 30, 2150, 0),      # Pillsbury Attack
            BookMove("f2f3", 80, 32, 24, 24, 2140, 0),       # Blackmar-Diemer Gambit
            BookMove("g2g3", 70, 28, 21, 21, 2130, 0),       # Catalan Opening
            BookMove("h2h3", 60, 24, 18, 18, 2120, 0),       # Anti-Grünfeld
            BookMove("a2a3", 50, 20, 15, 15, 2110, 0),       # Quiet development
            BookMove("b2b3", 45, 18, 14, 13, 2100, 0),       # Fianchetto
            BookMove("d1c2", 40, 16, 12, 12, 2090, 0),       # Queen development
            BookMove("c1d2", 35, 14, 11, 10, 2080, 0),       # Quiet development
            BookMove("f1d3", 30, 12, 9, 9, 2070, 0),         # Colle System
        ]
        ...
        # After 1.c4 - Black responses (MASSIVE EXPANSION - 20 moves)
        ...
        essential_openings[fen_english] = [
            ...
            BookMove("b8c6", 120, 48, 36, 36, 2130, 0),      # Nimzowitsch-English
            BookMove("f7f5", 100, 40, 30, 30, 2120, 0),      # Dutch setup
            BookMove("d7d6", 80, 32, 24, 24, 2110, 0),       # King's Indian Attack
            BookMove("b7b6", 70, 28, 21, 21, 2100, 0),       # Queen's Indian setup
            BookMove("c8b7", 60, 24, 18, 18, 2090, 0),       # Nimzo-Larsen setup
            BookMove("a7a6", 50, 20, 15, 15, 2080, 0),       # Great Snake
            BookMove("h7h6", 45, 18, 14, 13, 2070, 0),       # Waiting move
            BookMove("b8a6", 40, 16, 12, 12, 2060, 0),       # Mikenas Defense
            BookMove("a7a5", 35, 14, 11, 10, 2050, 0),       # Ware Defense
            BookMove("h7h5", 30, 12, 9, 9, 2040, 0),         # Grob Defense
            BookMove("c8f5", 25, 10, 8, 7, 2030, 0),         # London System setup
            BookMove("f7f6", 22, 9, 7, 6, 2020, 0),          # Barnes Defense
            BookMove("g8h6", 20, 8, 6, 6, 2010, 0),          # Lemming Defense
            BookMove("b7b5", 18, 7, 6, 5, 2000, 0),          # Polish Defense
        ]
        
        # === RETI OPENING (1.Nf3) ===
        
        # After 1.Nf3 - Black responses (MASSIVE EXPANSION - 20 moves)
        ...
        essential_openings[fen_reti] = [
            ...
            BookMove("b8c6", 150, 60, 45, 45, 2130, 0),      # Nimzowitsch Defense
            BookMove("c7c6", 120, 48, 36, 36, 2120, 0),      # Caro-Kann setup
            BookMove("b7b6", 100, 40, 30, 30, 2110, 0),      # Queen's Indian setup
            BookMove("f7f5", 80, 32, 24, 24, 2100, 0),       # Dutch Defense
            BookMove("d7d6", 70, 28, 21, 21, 2090, 0),       # Pirc Defense
            BookMove("e7e5", 60, 24, 18, 18, 2080, 0),       # King's Pawn
            BookMove("a7a6", 50, 20, 15, 15, 2070, 0),       # St. George Defense
            BookMove("h7h6", 45, 18, 14, 13, 2060, 0),       # Waiting move
            BookMove("b8a6", 40, 16, 12, 12, 2050, 0),       # Mikenas Defense
            BookMove("a7a5", 35, 14, 11, 10, 2040, 0),       # Ware Defense
            BookMove("h7h5", 30, 12, 9, 9, 2030, 0),         # Grob Defense
            BookMove("c8f5", 25, 10, 8, 7, 2020, 0),         # London System setup
            BookMove("f7f6", 22, 9, 7, 6, 2010, 0),          # Barnes Defense
            BookMove("g8h6", 20, 8, 6, 6, 2000, 0),          # Lemming Defense
            BookMove("b7b5", 18, 7, 6, 5, 1990, 0),          # Polish Defense
        ]
        ...
        logger.debug(f"Created comprehensive opening database with {len(essential_openings)} positions")
    ...
    def _add_detailed_variations(self, openings: Dict[str, List[BookMove]]):
        ...
        # MISSING: After 1.d4 f5 - Dutch Defense (CRITICAL GAP!)
        fen_1d4_f5 = "rnbqkbnr/ppppp1pp/8/5p2/3P4/8/PPP1PPPP/RNBQKBNR w KQkq - 0 2"
        openings[fen_1d4_f5] = [
            BookMove("g1f3", 500, 200, 150, 150, 2200, 0),   # Classical Dutch
            BookMove("c2c4", 400, 160, 120, 120, 2180, 0),   # English setup
            BookMove("b1c3", 300, 120, 90, 90, 2170, 0),     # Veresov Attack
            BookMove("c1g5", 250, 100, 75, 75, 2160, 0),     # Trompowsky setup
            BookMove("e2e3", 200, 80, 60, 60, 2150, 0),      # Colle System
            BookMove("g2g3", 150, 60, 45, 45, 2140, 0),      # King's Indian Attack
        ]
        
        # After 1.d4 f5 2.Nf3 - Dutch Defense continuation
        fen_dutch_nf3 = "rnbqkbnr/ppppp1pp/8/5p2/3P4/5N2/PPP1PPPP/RNBQKB1R b KQkq - 1 2"
        openings[fen_dutch_nf3] = [
            BookMove("g8f6", 500, 200, 150, 150, 2200, 0),   # Classical Dutch
            BookMove("e7e6", 400, 160, 120, 120, 2180, 0),   # Stonewall Dutch
            BookMove("d7d6", 300, 120, 90, 90, 2170, 0),     # Modern Dutch
            BookMove("b8c6", 200, 80, 60, 60, 2160, 0),      # Unusual development
            BookMove("c7c6", 150, 60, 45, 45, 2150, 0),      # Solid setup
        ]
        
        # After 1.d4 f5 2.c4 - English setup vs Dutch
        fen_dutch_c4 = "rnbqkbnr/ppppp1pp/8/5p2/2PP4/8/PP2PPPP/RNBQKBNR b KQkq - 0 2"
        openings[fen_dutch_c4] = [
            BookMove("g8f6", 500, 200, 150, 150, 2200, 0),   # Classical Dutch
            BookMove("e7e6", 400, 160, 120, 120, 2180, 0),   # Stonewall Dutch
            BookMove("d7d6", 300, 120, 90, 90, 2170, 0),     # Modern Dutch
            BookMove("b8c6", 200, 80, 60, 60, 2160, 0),      # Flexible
        ]
        ...
        logger.debug(f"Added detailed variations and continuations, total positions: {len(openings)}")
    ...
    def _collect_moves_from_sources(self, fen: str) -> List[BookMove]:
        """Collect moves from all available sources with hybrid Lichess integration"""
        all_moves = {}  # move_uci -> BookMove
        local_moves_found = False
        ...
        # 3. Check if we should use Lichess (hybrid logic)
        use_lichess = self._should_use_lichess(all_moves, local_moves_found)
        
        if use_lichess and self.lichess_client:
            lichess_moves = self._get_lichess_moves(fen)
            if lichess_moves:
                # Integrate Lichess moves
                for lichess_move in lichess_moves:
                    if lichess_move.move in all_moves:
                        # Combine with existing local data
                        existing = all_moves[lichess_move.move]
                        # Prefer Lichess data if it has significantly more games
                        if lichess_move.weight > existing.weight * 2:
                            lichess_move.source = "combined"
                            all_moves[lichess_move.move] = lichess_move
                        else:
                            # Keep local data but mark as combined
                            existing.source = "combined"
                    else:
                        # Add new move from Lichess
                        all_moves[lichess_move.move] = lichess_move
                
                # Update statistics
                if local_moves_found:
                    self.stats['combined_hits'] += 1
                else:
                    self.stats['lichess_hits'] += 1
            else:
                # Lichess failed, using local only
                if local_moves_found:
    ...
    def _should_use_lichess(self, local_moves: Dict[str, BookMove], local_found: bool) -> bool:
        """Determine if we should fetch from Lichess"""
        if not self.enable_lichess or not self.lichess_client:
            return False
        
        # Always use Lichess if no local data
        if not local_found:
            return True
        
        # Use Lichess if local data is insufficient
        if local_moves:
            top_local_move = max(local_moves.values(), key=lambda m: m.weight)
            if top_local_move.weight < self.prefer_local_threshold:
                return True
        
        return False
    
    def _get_lichess_moves(self, fen: str) -> List[BookMove]:
        """Get moves from Lichess masters database"""
        try:
            lichess_data = self.lichess_client.get_position_data(fen)
            if lichess_data:
                return self.lichess_client.convert_to_book_moves(lichess_data)
        except Exception:
            pass
    ...
    def _select_move_with_rotation(self, fen: str, moves: List[BookMove]) -> Optional[BookMove]:
        ...
        # Check if we should rotate (add variety) - now more aggressive
        should_rotate = (
            random.random() < self.rotation_factor or  # Always consider rotation
            len(position_history) > 0 and len(moves) > 1  # Or if we have history and options
        )
        ...
        if should_rotate:
            ...
            for move in moves[:min(5, len(moves))]:  # Consider top 5 moves instead of 3
                last_played = self.last_move_times.get(fen, {}).get(move.move, 0)
                time_since_played = current_time - last_played
                
                # Consider move "fresh" if not played in last 3 games or 30 minutes (more aggressive)
                if move.move not in position_history[-3:] or time_since_played > 1800:
    ...
    def get_statistics(self) -> Dict[str, Any]:
        """Get hybrid system usage statistics"""
        total_requests = sum(self.stats.values())
        stats_with_percentages = self.stats.copy()
        
        if total_requests > 0:
            for key, value in self.stats.items():
                stats_with_percentages[f"{key}_percentage"] = (value / total_requests) * 100
        
        stats_with_percentages['total_requests'] = total_requests
        stats_with_percentages['lichess_enabled'] = self.enable_lichess
        stats_with_percentages['lichess_available'] = self.lichess_client is not None
        
        return stats_with_percentages
    
    def disable_lichess(self):
        """Disable Lichess integration"""
        self.enable_lichess = False
        if self.lichess_client:
            self.lichess_client.disable()
    
    def enable_lichess(self):
        """Enable Lichess integration"""
        if self.lichess_client:
            self.enable_lichess = True
            self.lichess_client.enable()
    
    def clear_lichess_cache(self):
        """Clear Lichess cache"""
        if self.lichess_client:
            self.lichess_client.cache.clear()
